...
function itemLike(id) {
  ...
  if (locUse) {
    ...
    locUser.replace(/\./g, "");
    db.ref("".concat(RTbase, "/").concat(id)).once("value", function (snapshot) {
...
function getItemLikes(id) {
  db.ref("".concat(RTbase, "/").concat(id, "/UserLikes")).once("value", function (snapshot) {
...
function saveRating(id, rateYo) {
  ...
  if (locUse) {
    var locUser = locUse.replace(/\./g, "");
    locUser.replace(/\./g, "");
    db.ref("".concat(RTbase, "/").concat(id, "/rating")).once("value", function (snapshot) {
      var itemRating = snapshot.val();
      var rated = rateYo.rateYo("rating");

      if (itemRating != null && itemRating["rating"] >= 0) {
        db.ref(RTbase + "/" + id + "/rating").update(_defineProperty({}, locUser, _defineProperty({}, "rated", rated)));
      } else {
        db.ref(RTbase + "/" + id + "/rating").update(_defineProperty({}, locUser, _defineProperty({}, "rated", rated)));
      }
    });
