...
function videoP(id, validation, vidNames, poster, catego) {
  ...
  if (validation == "playVid") {
    ...
    for (var i = 0; i < vidArrNames.length; i++) {
      if (vidArrNames[i].indexOf("1080") > -1) {
        var vidSrc1080 = $("<source />", {
          src: "/sites/".concat(clubName, "/Categories/").concat(catego, "/Content/").concat(vidArrNames[i], ".mp4"),
          label: "1080 HD",
          res: "1080",
          type: "video/mp4"
        });
    ...
    $(".imgItem").show();
    var songSrc = $("<source />", {
      src: "/sites/".concat(clubName, "/Categories/").concat(catego, "/Content/").concat(id, ".mp3"),
      type: "audio/mp3"
    });
