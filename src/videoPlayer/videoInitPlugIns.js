// Configuration of inicialization of multiple videos plugIns
// Unkown cretor

var videoInitPlug = function() {
		...
		(function(window, videojs) {
			var player = videojs(
				...
				function() {
					...
					player.on("resolutionchange", function() {
						player.play();
						// Teset: console.info('Source changed to %s', player.src())
						...
						if (label == "1080 HD") {
							...
							document
								.getElementsByClassName("player_container")[0]
								.appendChild(node);
						} else {
							hd = false;
							$(".vjs-icon-hd").remove();
			...
			player.panorama({
				...
				NoticeMessage: isMobile()
					? "por favor mueva su dispositivo"
					: "utilice el cursor para arrastrar y soltar el video",
				callback: function() {
			...
			function addNewButton(data) {
				...
				newLink.innerHTML =
					"<i class='fa " + data.icon + " line-height' aria-hidden='true'></i>";
				/* NewLink.innerHTML = "<span class='vjs-icon-" + data.icon + " vjs-control vjs-button '></span>" */
